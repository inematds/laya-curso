...
(function (window, document) {
  ...
  function rawSet(key, value) {
    ...
    try {
      ...
      notify("Almacenamiento lleno. Exporta tu recorrido para no perder datos.", 'warn');
      return false;
    }
  }
  ...
  function manifestModules() {
    ...
    for (var i = 0; i < man.tracks.length; i++) {
      ...
      for (var j = 0; j < mods.length; j++) {
        var md = mods[j];
        out.push({ id: String(md.id), track: trk, topics: (+md.topics || 0), title: md.title || ("Módulo " + md.id), href: md.href || null });
      }
    }
    return out;
  }
  ...
  function setReadButtonVisual(btn, isOn, sec) {
    ...
    if (lbl) lbl.textContent = isOn ? "Leído" : "Marcar como leído";
    else if (!btn.querySelector('svg') && !btn.children.length) {
      btn.textContent = isOn ? "Leído" : "Marcar como leído";
    }
  ...
  function maybeCelebrate() {
    ...
    if (pr.total > 0 && pr.done === pr.total) {
      ...
      if (!meta.completedAt) {
        ...
        setMeta(meta);
        notify("Curso completado. Felicidades por tu esfuerzo.", 'ok');
      }
    }
  }
  ...
  function highlight(range, opts) {
    ...
    if (!block) { notify("Selecciona texto dentro de un párrafo que admita anotaciones.", 'warn'); return null; }
    if (endBlock && endBlock !== block) {
      // colapsa a selecao ao bloco inicial (E8: nao tenta cross-node)
      notify("La selección se limita a un párrafo.", 'warn');
    }
    var blockId = block.getAttribute('data-inema-block');
    if (!blockId) { notify("Este fragmento no admite anotaciones.", 'warn'); return null; }
  ...
  function buildSelectionPopover() {
    ...
    pop.setAttribute('aria-label', "Anotar selección");
    ...
    SWATCHES.forEach(function (c) {
      ...
      b.setAttribute('aria-label', c === 'doubt' ? "Marcar como duda" : ("Color de resaltado: " + c));
    ...
    addBtn.textContent = "Añadir nota";
  ...
  function fallbackCopy(text) {
    try {
      ...
      notify(ok ? "Copiado." : "No se pudo copiar.", ok ? 'ok' : 'warn');
    } catch (e) {
      notify("No se pudo copiar.", 'warn');
    }
  }
  ...
  function renderJourney(mountEl) {
    ...
    var h = el('h2', 'inema-journey__title', "Mi recorrido");
    h.id = 'inema-journey-title';
    head.appendChild(h);
    var closeB = el('button', 'inema-journey__close', "Cerrar");
    closeB.type = 'button';
    closeB.setAttribute('aria-label', "Cerrar mi recorrido");
    ...
    secP.appendChild(el('h3', 'inema-journey__section-title', "Progreso"));
    ...
    if (man && man.tracks && man.tracks.length) {
      man.tracks.forEach(function (tk) {
        ...
        secP.appendChild(meterRow(tk.title || ("Ruta " + trk), progress('trilha:' + trk)));
        (tk.modules || []).forEach(function (md) {
          secP.appendChild(meterRow(md.title || ("Módulo " + md.id), progress('modulo:' + md.id)));
        });
      });
    } else {
      var tracks = uniqueAttr('[data-inema-track]', 'data-inema-track');
      tracks.forEach(function (tk) {
        secP.appendChild(meterRow("Ruta " + tk, progress('trilha:' + tk)));
      });
      var mods = uniqueAttr('[data-inema-module]', 'data-inema-module');
      mods.forEach(function (md) {
        secP.appendChild(meterRow("Módulo " + md, progress('modulo:' + md)));
      });
    }
    ...
    if (meta.lastTopicAnchor || meta.lastModuleHref) {
      ...
      secC.appendChild(el('h3', 'inema-journey__section-title', "Continuar donde lo dejé"));
      var resumeBtn = el('button', 'inema-journey__resume', "Volver al último punto");
    ...
    secD.appendChild(el('h3', 'inema-journey__section-title', "Dudas (" + doubts.length + ')'));
    var filterWrap = el('div', 'inema-journey__filters');
    var onlyOpen = el('label', null, null);
    var cb = document.createElement('input'); cb.type = 'checkbox';
    onlyOpen.appendChild(cb); onlyOpen.appendChild(document.createTextNode(" solo sin resolver"));
    ...
    function paintDoubts() {
      ...
      doubts.forEach(function (d) {
        ...
        var label = d.kind === 'topic'
          ? ("Tema: " + d.id)
          : (d.quote ? ('"' + truncate(d.quote, 90) + '"') : "Fragmento resaltado");
        ...
        li.appendChild(a);
        var tag = el('span', 'inema-journey__tag', "duda");
        tag.setAttribute('data-tag', 'duvida');
        li.appendChild(tag);
        if (d.note) li.appendChild(el('p', 'inema-journey__note', d.note));
        if (d.kind === 'topic') {
          var rb = el('button', 'inema-btn inema-btn--ghost', d.resolved ? "Reabrir" : "Marcar como resuelta");
          rb.type = 'button';
      ...
      if (!doubtList.children.length) doubtList.appendChild(el('li', 'inema-journey-empty', "No hay dudas."));
    }
    ...
    secN.appendChild(el('h3', 'inema-journey__section-title', "Notas y resaltados (" + allNotes.length + ')'));
    ...
    ['todas'].concat(SWATCHES).forEach(function (c) {
      var o = document.createElement('option'); o.value = c; o.textContent = {"todas": "todos", "yellow": "amarillo", "green": "verde", "blue": "azul", "pink": "rosa", "doubt": "duda"}[c] || c; selColor.appendChild(o);
    });
    nFilter.appendChild(el('span', null, "Color: "));
    ...
    function paintNotes() {
      ...
      allNotes.forEach(function (n) {
        ...
        if (n.color === 'doubt') {
          var ntag = el('span', 'inema-journey__tag', "duda");
          ntag.setAttribute('data-tag', 'duvida');
          li.appendChild(ntag);
        }
        if (n.orphan) li.appendChild(el('span', 'inema-journey-orphan', "nota sin ancla"));
        var actions = el('div', 'inema-journey-actions');
        if (!n.orphan) {
          var go = el('button', 'inema-btn inema-btn--ghost', "Ir al fragmento");
          go.type = 'button';
          go.addEventListener('click', function () { closeJourney(); jumpTo({ kind: 'note', id: n.id, blockId: n.blockId }); });
          actions.appendChild(go);
        }
        var del = el('button', 'inema-btn inema-btn--ghost', "Eliminar");
        del.type = 'button';
      ...
      if (!noteList.children.length) noteList.appendChild(el('li', 'inema-journey-empty', "No hay notas."));
    }
    ...
    secX.appendChild(el('h3', 'inema-journey__section-title', "Copia de seguridad"));
    ...
    impInput.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        if (res.ok) {
          notify("Importado: " + res.applied + " elementos.", 'ok');
          rehydrateAll();
          renderJourney(mountEl);
        } else {
          notify("Archivo no válido. No se ha cambiado nada.", 'warn');
        }
      };
    ...
    var resetBtn = el('button', 'inema-btn inema-danger', "Borrar todo");
    resetBtn.type = 'button';
    resetBtn.addEventListener('click', function () {
      if (window.confirm("¿Borrar TODO el progreso, las dudas y las notas de este curso? Esta acción no se puede deshacer.")) {
        resetCourse();
        rehydrateAll();
        renderJourney(mountEl);
        notify("Datos del curso borrados.", 'ok');
      }
    });
  ...
  function jumpTo(item) {
    ...
    if (target && target.scrollIntoView) {
      ...
      flashTarget(target);
    } else { navigateTopic(item.kind === 'topic' ? item.id : item.blockId); }
  }
  ...
  function saveCheckpoint(topicId) {
    if (!topicId && !topMostVisibleTopic()) return;
  ...
  function navigateTopic(topicId) {
    var match = /modulo-(\d+-\d+)(?:#topico-(\d+)|-t(\d+))/.exec(topicId || '');
    if (!match) return false;
    var man = getManifest(), dest = null;
    (man && man.tracks || []).forEach(function(t) { (t.modules || []).forEach(function(m) {
      if (m.id === match[1]) dest = m.href;
    }); });
    if (!dest) return false;
    var script = document.querySelector('script[src*="assets/learn.js"]');
    var root = new URL('../', script.src);
    var url = new URL(dest, root); url.hash = 'topico-' + (match[2] || match[3] || '1');
  ...
  function resume() {
    ...
    if (anchor && navigateTopic(anchor)) return true;
  ...
  function downloadJSON() {
    ...
    try {
      ...
      try {
        ...
        a2.click();
      } catch (e2) { notify("No se pudo exportar.", 'warn'); }
    }
  }
  ...
  function importJSON(text, opts) {
    ...
    if (parsed === undefined || typeof parsed !== 'object' || parsed === null) {
      res.errors.push("JSON no válido o ilegible.");
      return res;
    }
    if (typeof parsed.schemaVersion !== 'number' || parsed.schemaVersion > SCHEMA_VERSION) {
      // versao futura desconhecida: nao commita (evita corromper)
      if (typeof parsed.schemaVersion !== 'number') {
        res.errors.push("Falta schemaVersion.");
        return res;
      }
      // se for futura, ainda tentamos preservar via migrate, mas avisamos
      res.errors.push('schemaVersion ' + parsed.schemaVersion + " es posterior a la versión compatible (" + SCHEMA_VERSION + ').');
    }

    if (!Number.isInteger(parsed.schemaVersion) || parsed.schemaVersion < 1 ||
        (parsed.courseId && parsed.courseId !== S.courseId)) {
      res.errors.push("Versión no válida o archivo de otro curso."); return res;
    }
    var fields = ['read', 'doubts', 'notes', 'checks', 'meta'];
    for (var fi = 0; fi < fields.length; fi++) {
      var value = parsed[fields[fi]];
      if (value != null && (typeof value !== 'object' || Array.isArray(value))) {
        res.errors.push("Estructura no válida: " + fields[fi]); return res;
      }
    }
    for (var nb in (parsed.notes || {})) {
      if (!Array.isArray(parsed.notes[nb]) || parsed.notes[nb].some(function(n) {
        return !n || typeof n.id !== 'string' || typeof n.quote !== 'string';
      })) { res.errors.push("Notas no válidas."); return res; }
    }
    // Preserve extension fields across actual storage/export, not only migrate().
    var extra = mode === 'replace' ? {} : storageGet(key('extra'), {});
    var known = ['schemaVersion','courseId','exportedAt'].concat(fields);
    Object.keys(parsed).forEach(function(k) {
      if (known.indexOf(k) < 0 && k !== '__proto__' && k !== 'constructor' && k !== 'prototype' &&
          (mode === 'replace' || !Object.prototype.hasOwnProperty.call(extra, k))) extra[k] = parsed[k];
    });
    var incoming = migrate(parsed);

    if (mode === 'replace') {
      if (!opts.__confirmed) {
        var ok = window.confirm("¿Reemplazar TODOS los datos actuales con el archivo importado? Se perderá el progreso actual.");
        if (!ok) { res.errors.push("cancelado por el usuario"); return res; }
      }
      storageSet(key('extra'), extra);
  ...
  function paintCheckFeedback(id, choice, correct, def) {
    try {
      ...
      var msg = correct ? "Correcto. " : "Revisa: ";
  ...
  function onMarkClick(mark, e) {
    ...
    noteBtn.addEventListener('click', function () {
      ...
      var txt = window.prompt("Tu anotación:", cur);
      if (txt !== null) promoteToNote(id, txt);
      removeMarkMenu();
    });
    menu.appendChild(noteBtn);

    var delBtn = el('button', 'inema-btn inema-btn--ghost', "Eliminar");
    delBtn.type = 'button';
  ...
  function onSelPopClick(e) {
    ...
    if (act) {
      ...
      if (a === 'note' && rng) {
        var txt = window.prompt("Tu anotación:", '');
    ...
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        ...
        if (counter) {
          ...
          counter.textContent = "Sección " + (idx + 1) + " de " + topics.length;
        }
      });
  ...
  function init(opts) {
    ...
    if (!S.inited) {
      ...
      S.ephemeral = !ok;
      if (S.ephemeral) {
        notify("Modo de lectura: tu progreso no se guardará en este navegador.", 'warn');
      }

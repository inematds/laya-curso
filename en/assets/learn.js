...
(function (window, document) {
  ...
  function rawSet(key, value) {
    ...
    try {
      ...
      notify("Storage is full. Export your journey to avoid losing data.", 'warn');
      return false;
    }
  }
  ...
  function manifestModules() {
    ...
    for (var i = 0; i < man.tracks.length; i++) {
      ...
      for (var j = 0; j < mods.length; j++) {
        var md = mods[j];
        out.push({ id: String(md.id), track: trk, topics: (+md.topics || 0), title: md.title || ("Module " + md.id), href: md.href || null });
      }
    }
    return out;
  }
  ...
  function setReadButtonVisual(btn, isOn, sec) {
    ...
    if (lbl) lbl.textContent = isOn ? "Read" : "Mark as read";
    else if (!btn.querySelector('svg') && !btn.children.length) {
      btn.textContent = isOn ? "Read" : "Mark as read";
    }
  ...
  function maybeCelebrate() {
    ...
    if (pr.total > 0 && pr.done === pr.total) {
      ...
      if (!meta.completedAt) {
        ...
        setMeta(meta);
        notify("Course completed. Well done on your effort.", 'ok');
      }
    }
  }
  ...
  function highlight(range, opts) {
    ...
    if (!block) { notify("Select text within an annotatable paragraph.", 'warn'); return null; }
    if (endBlock && endBlock !== block) {
      // colapsa a selecao ao bloco inicial (E8: nao tenta cross-node)
      notify("Selection is limited to one paragraph.", 'warn');
    }
    var blockId = block.getAttribute('data-inema-block');
    if (!blockId) { notify("This passage cannot be annotated.", 'warn'); return null; }
  ...
  function buildSelectionPopover() {
    ...
    pop.setAttribute('aria-label', "Annotate selection");
    ...
    SWATCHES.forEach(function (c) {
      ...
      b.setAttribute('aria-label', c === 'doubt' ? "Mark as a question" : ("Highlight color: " + c));
  ...
  function fallbackCopy(text) {
    try {
      ...
      notify(ok ? "Copied." : "Could not copy.", ok ? 'ok' : 'warn');
    } catch (e) {
  ...
  function renderJourney(mountEl) {
    ...
    var h = el('h2', 'inema-journey__title', "My journey");
    h.id = 'inema-journey-title';
    head.appendChild(h);
    var closeB = el('button', 'inema-journey__close', "Close");
    closeB.type = 'button';
    closeB.setAttribute('aria-label', "Close my journey");
    ...
    secP.appendChild(el('h3', 'inema-journey__section-title', "Progress"));
    var cur = progress('curso');
    body.appendChild(secP);
    secP.appendChild(meterRow("Course", cur));
    ...
    if (man && man.tracks && man.tracks.length) {
      man.tracks.forEach(function (tk) {
        ...
        secP.appendChild(meterRow(tk.title || ("Track " + trk), progress('trilha:' + trk)));
        (tk.modules || []).forEach(function (md) {
          secP.appendChild(meterRow(md.title || ("Module " + md.id), progress('modulo:' + md.id)));
        });
      });
    } else {
      var tracks = uniqueAttr('[data-inema-track]', 'data-inema-track');
      tracks.forEach(function (tk) {
        secP.appendChild(meterRow("Track " + tk, progress('trilha:' + tk)));
      });
      var mods = uniqueAttr('[data-inema-module]', 'data-inema-module');
      mods.forEach(function (md) {
        secP.appendChild(meterRow("Module " + md, progress('modulo:' + md)));
      });
    }
    ...
    if (meta.lastTopicAnchor || meta.lastModuleHref) {
      ...
      secC.appendChild(el('h3', 'inema-journey__section-title', "Continue where I left off"));
      var resumeBtn = el('button', 'inema-journey__resume', "Return to last position");
    ...
    secD.appendChild(el('h3', 'inema-journey__section-title', "Questions (" + doubts.length + ')'));
    var filterWrap = el('div', 'inema-journey__filters');
    var onlyOpen = el('label', null, null);
    var cb = document.createElement('input'); cb.type = 'checkbox';
    onlyOpen.appendChild(cb); onlyOpen.appendChild(document.createTextNode(" unresolved only"));
    ...
    function paintDoubts() {
      ...
      doubts.forEach(function (d) {
        ...
        var label = d.kind === 'topic'
          ? ("Topic: " + d.id)
          : (d.quote ? ('"' + truncate(d.quote, 90) + '"') : "Highlighted passage");
        ...
        li.appendChild(a);
        var tag = el('span', 'inema-journey__tag', "question");
        tag.setAttribute('data-tag', 'duvida');
        li.appendChild(tag);
        if (d.note) li.appendChild(el('p', 'inema-journey__note', d.note));
        if (d.kind === 'topic') {
          var rb = el('button', 'inema-btn inema-btn--ghost', d.resolved ? "Reopen" : "Mark resolved");
          rb.type = 'button';
      ...
      if (!doubtList.children.length) doubtList.appendChild(el('li', 'inema-journey-empty', "No questions yet."));
    }
    ...
    secN.appendChild(el('h3', 'inema-journey__section-title', "Notes and highlights (" + allNotes.length + ')'));
    ...
    ['todas'].concat(SWATCHES).forEach(function (c) {
      var o = document.createElement('option'); o.value = c; o.textContent = {"todas": "all", "yellow": "yellow", "green": "green", "blue": "blue", "pink": "pink", "doubt": "question"}[c] || c; selColor.appendChild(o);
    });
    ...
    function paintNotes() {
      ...
      allNotes.forEach(function (n) {
        ...
        if (n.color === 'doubt') {
          var ntag = el('span', 'inema-journey__tag', "question");
          ntag.setAttribute('data-tag', 'duvida');
          li.appendChild(ntag);
        }
        if (n.orphan) li.appendChild(el('span', 'inema-journey-orphan', "note without an anchor"));
        var actions = el('div', 'inema-journey-actions');
        if (!n.orphan) {
          var go = el('button', 'inema-btn inema-btn--ghost', "Go to passage");
          go.type = 'button';
          go.addEventListener('click', function () { closeJourney(); jumpTo({ kind: 'note', id: n.id, blockId: n.blockId }); });
          actions.appendChild(go);
        }
        var del = el('button', 'inema-btn inema-btn--ghost', "Delete");
        del.type = 'button';
      ...
      if (!noteList.children.length) noteList.appendChild(el('li', 'inema-journey-empty', "No notes yet."));
    }
    ...
    var expBtn = el('button', 'inema-btn', "Export .json");
    expBtn.type = 'button';
    expBtn.addEventListener('click', downloadJSON);
    row.appendChild(expBtn);

    var impLabel = el('label', 'inema-btn', "Import .json");
    ...
    impInput.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        if (res.ok) {
          notify("Imported: " + res.applied + " items.", 'ok');
          rehydrateAll();
          renderJourney(mountEl);
        } else {
          notify("Invalid file. Nothing was changed.", 'warn');
        }
      };
    ...
    var resetBtn = el('button', 'inema-btn inema-danger', "Delete everything");
    resetBtn.type = 'button';
    resetBtn.addEventListener('click', function () {
      if (window.confirm("Delete ALL progress, questions and notes for this course? This cannot be undone.")) {
        resetCourse();
        rehydrateAll();
        renderJourney(mountEl);
        notify("Course data deleted.", 'ok');
      }
    });
  ...
  function downloadJSON() {
    ...
    try {
      ...
      try {
        ...
        a2.click();
      } catch (e2) { notify("Could not export.", 'warn'); }
    }
  }
  ...
  function importJSON(text, opts) {
    ...
    for (var nb in (parsed.notes || {})) {
      ...
      })) { res.errors.push("Invalid notes."); return res; }
    }
    ...
    if (mode === 'replace') {
      if (!opts.__confirmed) {
        var ok = window.confirm("Replace ALL current data with the imported file? Current progress will be lost.");
        if (!ok) { res.errors.push("cancelled by user"); return res; }
      }
  ...
  function paintCheckFeedback(id, choice, correct, def) {
    try {
      ...
      var msg = correct ? "Correct. " : "Review: ";
  ...
  function onMarkClick(mark, e) {
    ...
    var noteBtn = el('button', 'inema-btn inema-btn--ghost', "Note");
    noteBtn.type = 'button';
    noteBtn.addEventListener('click', function () {
      var f = findNote(id);
      var cur = f && f.rec.note ? f.rec.note : '';
      var txt = window.prompt("Your note:", cur);
      if (txt !== null) promoteToNote(id, txt);
      removeMarkMenu();
    });
    menu.appendChild(noteBtn);

    var delBtn = el('button', 'inema-btn inema-btn--ghost', "Delete");
    delBtn.type = 'button';
  ...
  function onSelPopClick(e) {
    ...
    if (act) {
      ...
      if (a === 'note' && rng) {
        var txt = window.prompt("Your note:", '');
    ...
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        ...
        if (counter) {
          ...
          counter.textContent = "Section " + (idx + 1) + " of " + topics.length;
        }
      });
  ...
  function init(opts) {
    ...
    if (!S.inited) {
      ...
      S.ephemeral = !ok;
      if (S.ephemeral) {
        notify("Reading mode: your progress will not be saved in this browser.", 'warn');
      }

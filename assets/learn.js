...
(function (window, document) {
  ...
  function importJSON(text, opts) {
    ...
    if (!Number.isInteger(parsed.schemaVersion) || parsed.schemaVersion < 1 ||
      ...
      res.errors.push('Versão inválida ou arquivo de outro curso.'); return res;
    }
    var fields = ['read', 'doubts', 'notes', 'checks', 'meta'];
    for (var fi = 0; fi < fields.length; fi++) {
      var value = parsed[fields[fi]];
      if (value != null && (typeof value !== 'object' || Array.isArray(value))) {
        res.errors.push('Estrutura inválida: ' + fields[fi]); return res;
      }
    }
    for (var nb in (parsed.notes || {})) {
      ...
      })) { res.errors.push('Notas inválidas.'); return res; }
    }
